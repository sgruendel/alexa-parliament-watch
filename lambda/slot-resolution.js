/** Resolve a custom slot without assuming Alexa supplied a value or a match. */
export function resolveSlot(slot, { requireId = false } = {}) {
    ...
    if (matches.length === 0) {
        return {
            status: authorities.some((authority) => authority.status?.code === 'ER_SUCCESS_NO_MATCH')
                ? 'unknown'
                : 'error',
        };
    }
    const values = matches
        .flatMap((authority) => authority.values ?? [])
